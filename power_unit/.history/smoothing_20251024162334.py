...
#     print("status:", problem.status)
    ...
    # Anchor initial state at t=0 (otherwise infinite solutions exist)
    # Use least-squares estimate from first observation
    C_pinv = np.linalg.pinv(C)
    z0_init = C_pinv @ y[0, :]
    constraints.append(Z[0, :] == z0_init)
    
    # Add dynamics constraints for ENTIRE trajectory
    for t in range(T):
    ...
    obj_terms = []
    # Fit future observations only (T_past to T-1)
