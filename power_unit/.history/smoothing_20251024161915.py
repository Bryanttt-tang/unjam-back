...
#     print("status:", problem.status)
    ...
    Z = cp.Variable((T_fut+1, n))  # Only need states from T_past onwards
    constraints = []
    
    # Anchor initial state at T_past using pseudo-inverse of C
    C_pinv = np.linalg.pinv(C)
    z0_init = C_pinv @ y[T_past, :]
    constraints.append(Z[0, :] == z0_init)
    
    # Add dynamics constraints for future only
    ...
    obj_terms = []
    # Fit future observations only (T_fut observations)
    for t in range(T_fut):
        resid = C @ Z[t, :] - y[T_past + t, :]
    ...
    Z_est = Z.value   # shape (T_fut+1, n)
    
    # Check future reconstruction (this should match the residual)
    y_fut_est = (C @ Z_est[:-1, :].T).T  # shape (T_fut, p)
    print("Future observation reconstruction error (Frobenius norm):", np.linalg.norm(y_fut_est - y[T_past:T, :], ord='fro'))
    print("Future observation reconstruction error (relative):", np.linalg.norm(y_fut_est - y[T_past:T, :], ord='fro')/np.linalg.norm(y[T_past:T, :], ord='fro'))
    
    # Also check future reconstruction
    z_fut_est = Z_est[T_past:T, :]
    y_fut_est = (C @ z_fut_est.T).T
    print("Future observation reconstruction error:", np.linalg.norm(y_fut_est - y[T_past:T, :], ord='fro'))
