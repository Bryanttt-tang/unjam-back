...
if __name__ == "__main__":
   ...
   # Get the optimum w*
    ...
    errors = []
    projected_point_alternating1 = F.alternating_projections2(F.proj_h_sub, w_ran, num_iterations=1)
    projected_point_alternating10 = F.alternating_projections2(F.proj_h_sub, w_ran, num_iterations=2)
    ...
    print('difference between 1 and 10 iterations:',np.linalg.norm(projected_point_alternating1 - projected_point_alternating10))
    # # direct_projection=h_total@np.linalg.pinv(h_total)@w_ran
    # with ThreadPool(processes=8) as pool:
    #     for ite in tqdm(range(1,26)):
    #         projected_point_alternating = F.alternating_projections2(F.proj_h_sub, w_ran, pool, num_iterations=ite)
    #         error = np.linalg.norm(direct_projection-projected_point_alternating)
    #         errors.append(error)
    # # plt.cla()
    # plt.figure(figsize=(10, 6))
