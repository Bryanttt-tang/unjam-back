...
#     print("status:", problem.status)
    ...
    # Add constraints for past observations (all observed)
    for t in range(T_past):
        constraints.append(C @ Z[t, :] == y[t, :])
    ...
    alpha = 1e-6
    objective = cp.Minimize(cp.sum(obj_terms) + alpha*cp.sum_squares(Z))
    ...
    print("Past observation reconstruction error (should be small):", np.linalg.norm(y_past_est - y[:T_past, :], ord='fro')/np.linalg.norm(y[:T_past, :], ord='fro'))
