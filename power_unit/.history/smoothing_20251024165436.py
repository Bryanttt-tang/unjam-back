...
if __name__ == "__main__":
    ...
    # Make C full column rank and C.T @ C = I
    # p >= n required
    Qc, _ = np.linalg.qr(np.random.randn(p, n))
    C = Qc  # shape (p, n), C.T @ C = I
    ...
    print("Hankel matrix shape:", H_y.shape)
...
#     print("status:", problem.status)
    ...
    # Add dynamics constraints and anchor initial state
    constraints.append(Z[0, :] == z_true[0, :])  # anchor
    for t in range(T):
