...
# --- Compute cp(C, D) and Friedrichs angle ---
# Step 1: Orthonormal basis for C (from Hankel)
H = h_dis  # shape (72, 28)
A, _ = np.linalg.qr(H)  # shape (72, 28)

# Step 2: Orthonormal basis for D (image of D_proj)
# Assume P_kerG is already defined as (6, 6) projector onto ker(G)
# If not, compute it as needed
P_kerG = ... # define or compute this projector
D_proj = np.kron(np.eye(12), P_kerG)  # shape (72, 72)
eigvals, eigvecs = np.linalg.eigh(D_proj)
B = eigvecs[:, np.isclose(eigvals, 1)]  # shape (72, r_D)

# Step 3: SVD of A.T @ B
U, s, Vh = np.linalg.svd(A.T @ B)
sigma_max = s[0]  # largest singular value

# Step 4: cp(C, D) and Friedrichs angle
cp_CD = sigma_max
theta_F = np.arccos(cp_CD)
print(f"cp(C, D) = {cp_CD:.4f}")
print(f"Friedrichs angle θ_F = {np.degrees(theta_F):.2f} degrees")